...
const inputData = JSON.parse(fs.readFileSync(path.join(__dirname, 'treeValues', 'priceMT.json'), 'utf8'));
const values = inputData.values;

const tree = constructMerkleTree(values, ['string', 'uint256', 'bytes32', 'uint256', 'uint256']);
const merkleRoot = tree.root;

// Derive hashType from the merkle tree's name
const merkleTreeName = 'priceMT';
...
const outputPath = path.join(outputDir, 'priceMT.json');
