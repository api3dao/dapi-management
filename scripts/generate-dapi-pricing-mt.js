...
const { createDapiPricingMerkleTree, deriveDataFeedId } = require('./utils');
...
async function generateDapiPricingMT() {
  ...
  WITH "DataComparison" AS (
    SELECT
      ...
      "timestamp",
      "encodedValue",
      CAST("decodedValue" AS numeric),
      "signature",
      "dataFeedId",
      CAST(LEAD("decodedValue") OVER (PARTITION BY "airnode", "templateId" ORDER BY "timestamp") AS numeric) AS "nextDecodedValue"
    FROM
      "SignedData"
  ),
  "DailyUpdateCounts" AS (
    SELECT
      ...
      CAST(GREATEST(750, COUNT(*) FILTER (WHERE (CAST("decodedValue" AS numeric) > 1.0025 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric))
        OR CAST("decodedValue" AS numeric) < 0.9975 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric)))
        AND "nextDecodedValue" IS NOT NULL)) AS INTEGER) AS "updateCount_0.25_120",
      CAST(GREATEST(1, COUNT(*) FILTER (WHERE (CAST("decodedValue" AS numeric) > 1.0025 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric))
        OR CAST("decodedValue" AS numeric) < 0.9975 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric)))
        AND "nextDecodedValue" IS NOT NULL)) AS INTEGER) AS "updateCount_0.25_86400",
      CAST(GREATEST(1, COUNT(*) FILTER (WHERE (CAST("decodedValue" AS numeric) > 1.005 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric))
        OR CAST("decodedValue" AS numeric) < 0.995 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric)))
        AND "nextDecodedValue" IS NOT NULL)) AS INTEGER) AS "updateCount_0.5_86400",
      CAST(GREATEST(1, COUNT(*) FILTER (WHERE (CAST("decodedValue" AS numeric) > 1.01 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric))
        OR CAST("decodedValue" AS numeric) < 0.99 * COALESCE(CAST("nextDecodedValue" AS numeric), CAST("decodedValue" AS numeric)))
  ...
  const dataFeedUpdateCountsWithOptions = dataFeedUpdateCounts.rows
    .map(({ dataFeedId, day, ...updateCountFields }) => {
      ...
      return { dataFeedName: dataFeedNamesByDataFeedId[dataFeedId], day, updateCounts };
    })
